...
const { applyCompatNollup } = require('./compat-nollup')
const { applyAutoCreate } = require('./autocreate')
...
module.exports = (options = {}) => {
  ...
  if (compatNollup) {
    ...
    return {
      ...
      options: pipe(
        ...
        applyAutoCreate(
          autoCreateOpt && { compatNollup: true, ...autoCreateOpt }
        )
      ),
    }
  }
  ...
  if (compatModuleHot) {
    ...
    return {
      ...
      options: applyAutoCreate(autoCreateOpt),
      transform(code, id) {
        if (/\.js$/.test(id)) {
          return {
  ...
  const hooks = {
    ...
    options: options => {
      const patched = applyAutoCreate(autoCreateOpt)({ ...options })
      // preserveModules
